...
width = 0 #Orig 0
height = 0
ContadorEntradas = 0 #Starting entrance counter value
ContadorSaidas = 0 # Starting exit counter value

AreaContornoLimiteMin = 230 #Minimum Contour Area Orig 100
ThresholdBinarizacao = 55  #Sensitivity, binarisation threshold, default 90
OffsetLinhasRef = 25 #Zone size, smaller is narrower, default 60 
timeout = time.time() + 120*1  # 1 minute from now, the program will shutdown, for testing purposes or else.
...
while True:
    ...
    print ("Detected: "+str(QtdeContornos))

    #Writes on screen the number of people who enters or leaves the watched area.
    cv2.putText(Frame, "Coming In: {}".format(str(ContadorEntradas)), (10, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (250, 0, 1), 2)
    
    cv2.putText(Frame, "Going Out: {}".format(str(ContadorSaidas)), (10, 70),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    #scroll_message(" In: " + str(ContadorSaidas))
    cv2.imshow("Original", Frame)
    #scroll_message(" In: " + str(ContadorEntradas) + " OUT: " + str(ContadorSaidas))
...
# Upload the values to the Adafruit dashboard

aio.send_data('temperature', ContadorSaidas)
aio.send_data('humidity', ContadorEntradas)

#Program writes the count to a file.
f = open( 'Visitors.txt', 'w' ) #File path and it's name with extension, to write to.
f.write( 'Coming In = ' + repr(ContadorEntradas) + '\n' ) #Variables to write
f.write( 'Going Out = ' + repr(ContadorSaidas) + '\n' ) #Variables to write
f.close()
